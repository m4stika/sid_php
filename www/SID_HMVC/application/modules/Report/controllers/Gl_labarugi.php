...
* 
*/
class Gl_labarugi extends MY_Controller {
	...
	public function index() {
		...
		$ColoredTable = function() use ($pdf) {
	        $labarugi = $this->report_model->get_GLLabarugi();
...
//Cell($pdf->PageWidth, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M')
		...
		$pdf->resetPageWidth();
		$pdf->SetTitle('SID | Profit && Lost');
		$pdf->AddPage();
		
		// Generate Report Header
		$pdf->setCellPaddings(0,1,0,1);
		$pdf->reportHeader('Profit ( Lost )', 'C', 'B', sidlib::rgbColor(TITLE_COLOR), 20, 4, 'B');
