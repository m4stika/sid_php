...
class Sar_lappenjualan_xls extends MY_Controller {
	...
	public function index() {		
		...
		$recordobject = $this->report_model->get_penjualan();
//noid, statusbooking, "" as statusbookingname, blok, nokavling, typerumah, luasbangunan, luastanah, kelebihantanah, sudut, hadapjalan, fasum		
        ...
        foreach ($recordobject as $key => $value) {
        	$addCell('B',$row,array(' '.$value->nopesanan, $value->namapemesan, $value->blok.' - '.$value->nokavling), $fill);
        ...
        $header = array('title'=>array('No. Booking'."\n".'Konsumen'."\n".'Kavling',	
        				'Tgl Transaksi'."\n".'Status'."\n".'Tgl Akad',
        				'Hrg Dasar'."\n".'Plafon KPR'."\n".'Uang Muka', 
        				'Booking Fee'."\n".'KLT'."\n".'Sudut', 
        				'Hadap Jalan'."\n".'Fasum'."\n".'Redesign', 
		...
		$objPHPExcel->setFontStyle('B3:'.$highestColumm.'4',10,true,HEADER_TEXT_COLOR);
        ...
        //Fill Footer
		$objPHPExcel->setFillColor('B'.$row.':'.$highestColumm.($row+2),'yellow-light');
        $objPHPExcel->setFontStyle('B'.$row.':'.$highestColumm.($row+2),8,true);
