var journalassign = function() {
	var handleForm = function(){
		var wrapper = $('#row_assign');
		var table = wrapper.find('#table_assign');
		...
		grid.init({
            ...
            dataTable: {
            	...
            	"ajax": {
            		type: "post",      
                    dataType: "json",              
					url: siteurl+"Accounting/assign/get_assign",
					data: function (d) {
	    ...
	    wrapper.on('click','.save', function(){
	    	...
	    	App.blockUI({target: table, overlayColor: 'none', cenrerY: true, animate: true});
	    	var rows = [];
            table.find('tbody > tr > td:nth-child(1) input[type="checkbox"]:checked', table).each(function() {
                var data = oTable.row($(this).closest('tr')).data();
                rows.push({linkid: data.linkid,
	    	...
	    	$.ajax({
				...
				dataType: 'JSON',
				//async: false,
				url: siteurl+'Accounting/assign/save_assign',
				data: {record: JSON.stringify(rows)},
				error: function(xhr, error, thrown) {									
					wrapper.find('.info').html(xhr.responseText);
					App.unblockUI(table);
            ...
            if ( row.child.isShown() ) {
                ...
                oInnerTable = $("#exampleTable_" + row.data().linkid).dataTable({
                    ...
                    "ajax": {
	                    ...
	                    url: siteurl+"Accounting/assign/get_assign_detail",
	                    data: function (d) {
